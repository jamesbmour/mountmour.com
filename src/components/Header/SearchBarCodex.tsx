...
import type { ChangeEventHandler, FormEventHandler, KeyboardEventHandler, MouseEventHandler } from 'react'
...
const SearchBar = () => {
  ...
  const results = useMemo<SearchResult[]>(() => {
    ...
    return fuseInstance
      .search(query.trim())
      .slice(0, 6)
      .map((result: FuseResult<SearchDocument>) => ({ ...result.item, score: result.score ?? 0 }))
  }, [fuseInstance, query])
  ...
  const navigateToSearchPage = (searchQuery: string): void => {
  ...
  const handleResultSelect = (url: string): void => {
    if (!url) return
    goToUrl(url)
  }
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <form onSubmit={handleSubmit as any} role="search" className="relative">
        <input
          ref={inputRef}
          type="search"
          value={query}
      ...
      {isInputFocused && query.trim() && results.length > 0 && (
        ...
        >
          {results.map((result: SearchResult, index: number) => (
            <li key={result.id} role="option" aria-selected={activeIndex === index ? true : false}>
              <button
                type="button"
